...
class MRF89XR: public SPI
{
    public:
        ...
        int Send_Packet(char txPacketLength, const char* txPacket)
        {
            ...
            for(int i=0; i< txPacketLength; i++)
            {
                ...
                pc.printf("writing value: %c \n",txPacket[i]);
                wait_ms(10);
            }
            
            SetRFMode(RF_TRANSMITTER);
            wait_ms(10);//while(!IRQ1_Received);  //Wait until TX Done interrupt and restore the RF state to standby mode
            SetRFMode(RF_STANDBY);
            SetRFMode(RF_RECEIVER);
            
            char j = RegisterRead(REG_IRQPARAM0);
            RegisterWrite(REG_IRQPARAM0, (j | 0x01));
    ...
    private:
        ...
        */
        ...
        void WriteInitConfigRegister(void)
        {
            for(int i = 0; i<32; i++)
            {
                RegisterWrite(i,InitConfigRegister[i]);
            }
            
        }
        
        
        void SetInitConfigRegister(void)
        {
            char tempArray[] = {
        /* 0 */             CHIPMODE_STBYMODE | FREQBAND_902 | VCO_TRIM_01, 
        ...
        /* 31 */            FIFO_AUTOCLR_ON | FIFO_STBY_ACCESS_WRITE 
            ...
            for(int x=0; x<32; x++)
            {
                InitConfigRegister[x] = tempArray[x];
            }
